...
const updateNews = () => {
  const randomAdjective = globalAdjectives[Math.floor(Math.random()*globalAdjectives.length)];
  const randomBuzzWord = globalBuzzWords[Math.floor(Math.random()*globalBuzzWords.length)];
  const newNews = `${capitalizeFirstLetter(randomAdjective)} ${randomBuzzWord}`;
  financialNews.push(newNews);
}

[1,2,3,4,5].forEach(updateNews);
this.setInterval(updateNews, 1000);
